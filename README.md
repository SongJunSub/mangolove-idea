...
불필요 — 최신 Homebrew는 `--no-quarantine`을 제거했고 cask가 대신 처리). 서드파티 tap이라 첫 설치
시 Homebrew의 tap-trust 안내가 한 번 표시됩니다. 갱신·삭제는 `brew upgrade --cask mangolove-idea` /
`brew uninstall --cask mangolove-idea`. (tap: [SongJunSub/homebrew-tap](https://github.com/SongJunSub/homebrew-tap))
