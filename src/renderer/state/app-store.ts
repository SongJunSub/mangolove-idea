 ...
 * confirm dialog (the switch tears this window's agents/servers down + reloads). Busy iff any
 * worktree has an agent turn in flight ('starting' before the PTY is up, 'running' mid-turn).
 * 'idle'/'exited'/'error' are NOT busy — nothing live to interrupt. A live dev SERVER is
 * intentionally NOT counted: it holds no unsaved work and is cheap to restart against the new
 * repo. The open editor is NOT counted either — auto-save flushes it before the switch, so its
 * buffer is already persisted. No React, no IO — unit tested directly; App is the only caller.
 */
export function isRepoBusy(statuses: ReadonlyMap<string, WorktreeRowStatus>): boolean {
