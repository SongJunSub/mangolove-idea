...
> ⚠️ **이 문서는 2026-06-16 브레인스토밍 시점의 기록(historical)이다.** 그 이후 v1+v2가 전부
> 구현됐고, 일부 결정은 진화했다(예: "서버 한 번에 하나" → v2 병렬 서버, b-lite → b-full 추가).
> **현재 구현 구조는 [ARCHITECTURE.md](ARCHITECTURE.md)를 보라.** 이 문서는 왜 이렇게 시작했는지의
> 맥락 보존용으로 남긴다.
